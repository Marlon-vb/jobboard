...
const RX_ENGLISH_OK =
  /\b(english[-\s]?speaking|fluent\s+(in\s+)?english|english\s+is\s+(our|the)\s+(working|company|official|main)\s+language|working\s+language\s+is\s+english|we\s+(work|communicate|operate)\s+in\s+english|english\s+(language\s+)?(is\s+)?required|good\s+(command|knowledge|level)\s+of\s+english|(no|without)\s+(german|french|dutch|spanish|italian|polish)\s+(language\s+)?(skills?\s+)?(is\s+)?(required|needed|necessary)|(sehr\s+)?gute\s+englischkenntnisse|flie[sß]end\w*\s+englisch|verhandlungssicher\w*\s+englisch|englisch\s+in\s+wort\s+und\s+schrift|anglais\s+courant|ma[îi]trise\s+de\s+l['’]anglais|anglais\s+professionnel|ingl[ée]s\s+(fluido|alto)|(nivel\s+alto|dominio)\s+del?\s+ingl[ée]s|inglese\s+fluente|ottima\s+conoscenza\s+dell['’]inglese|vloeiend\s+engels|goede\s+beheersing\s+van\s+het\s+engels|bieg[łl]\w*\s+angielski\w*|znajomo[śs][ćc]\s+j[ęe]zyka\s+angielskiego)\b/i;
...
function stopwordCounts(text) {
  ...
  return { en, other, length: t.length };
}

function looksEnglish(text) {
  const { en, other, length } = stopwordCounts(text);
  if (length < 40) return false;
  return en >= 4 && en > other;
}

// Confidently non-English: enough text, clearly dominated by another language's
// stopwords. Conservative on purpose — short or ambiguous text returns false,
// so an English title with no description is never misclassified.
export function looksNonEnglish(text = '') {
  const { en, other, length } = stopwordCounts(text);
  if (length < 60) return false;
  return other >= 5 && other > en;
}
...
// The board's language-accessibility rule: ads written fully in another
// language are dropped UNLESS they explicitly welcome English speakers AND
// offer remote capability (Remote or Hybrid). English/ambiguous ads pass.
export function isAccessibleToEnglishSpeakers(adText = '', english = '', remote = '') {
  if (!looksNonEnglish(adText)) return true;
  return english === 'English-friendly' && (remote === 'Remote' || remote === 'Hybrid');
}
